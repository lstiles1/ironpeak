...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-700 ${isScrolled ? 'glass-nav py-3' : 'bg-transparent py-8'}`}>
          ...
          <div className="hidden md:flex items-center space-x-10">
            {NAV_ITEMS.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className="relative text-brand-silver hover:text-white text-[10px] font-bold uppercase tracking-[0.4em] transition-all group py-2"
              >
                {item.label}
